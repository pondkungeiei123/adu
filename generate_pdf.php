...
class PDF extends TCPDF
{
    ...
    {
        $rightLowFreq = [
            floatval($row['right_500']),
            floatval($row['right_1000']),
            floatval($row['right_2000'])
        ];
        $leftLowFreq = [
            floatval($row['left_500']),
            floatval($row['left_1000']),
            floatval($row['left_2000'])
        ];
        $rightHighFreq = [
            floatval($row['right_3000']),
            floatval($row['right_4000']),
            floatval($row['right_6000']),
            floatval($row['right_8000'])
        ];
        $leftHighFreq = [
            floatval($row['left_3000']),
            floatval($row['left_4000']),
            floatval($row['left_6000']),
            floatval($row['left_8000'])
        ];
        ...
        if ($maxRightLow <= 25 && $maxRightHigh <= 25 && $maxLeftLow <= 25 && $maxLeftHigh <= 25) {
        ...
        } elseif ($maxRightLow > 25 && $maxRightHigh > 25 && $maxLeftLow > 25 && $maxLeftHigh > 25) {
            ...
            $summary = "พนักงานมีการได้ยินผิดปกติในบางความถี่";
        }
    ...
    public function AudiogramReport($audio_records)
    {
        ...
        foreach ($audio_records as $index => $row) {
            ...
            // ข้อมูลส่วนตัว
            $this->SetFont('THSarabunNew', '', 16);
            $this->Cell(60, 8, 'สถานประกอบการ: ' . $row['establishment'], 0);
            ...
            $this->SetFont('THSarabunNew', '', 16);
            $this->MultiCell(0, 8, $hearingResult['summary'], 0, 'L');
            $this->Ln(5);
            ...
            $this->Cell(0, 8, '                                                                                                      ...................................................', 0, 1, );
            $this->Ln(3);
            $this->Cell(0, 8, '                                                                                                       (พญ.นภัชฌา มโนรัตน์ ว.42781 )', 0, 1, '');
